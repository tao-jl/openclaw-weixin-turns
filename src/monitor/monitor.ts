...
export async function monitorWeixinProvider(opts: MonitorWeixinOpts): Promise<void> {
  ...
  const dispatchMessage = async (
    full: Parameters<typeof processOneMessage>[0],
    turnMeta?: { count: number; peerId: string },
  ) => {
    const fromUserId = full.from_user_id ?? "";
    const cachedConfig = await configManager.getForUser(fromUserId, full.context_token);

    if (turnMeta && turnMeta.count > 1) {
      aLog.info(
        `turn dispatch: peer=${turnMeta.peerId} merged=${turnMeta.count} messages`,
      );
    }
  ...
  const turnManager = new ConversationTurnManager({
    onReady: (full, metadata) => {
      dispatchQueue = dispatchQueue
        ...
        .catch((err) => {
          errLog(`weixin turn dispatch error: ${String(err)}`);
          aLog.error(`turn dispatch error: ${String(err)}, stack=${(err as Error).stack}`);
        });
    },
  });
  ...
  while (!abortSignal?.aborted) {
    try {
      ...
      for (const full of list) {
        ...
        // allowFrom filtering remains delegated to processOneMessage via the framework
        // authorization pipeline. The turn layer only decides when a text message
        // is ready to enter that existing pipeline.
        turnManager.accept(full);
      }
    } catch (err) {
